...
export default function EventShow({ event, otherEvents, sertifikat }: PageProps) {
    ...
    const renderTabContent = () => {
        switch (activeTab) {
            ...
            case 'sertifikat':
                if (event.sertifikat_events.length === 0) {
                    return <h1>Sertifikat belum dibuat</h1>;
                }

                return sertifikat === null ? (
                    <div>
                    ...
                    <div>
                        <div className="mb-2">
                            <h1>Anda sudah upload sertif</h1>
                            <img src={`/storage/${sertifikat.photo_sertifikat}`} alt="Sertifikat" />
                        </div>
                        {sertifikat.paraf_bem === 'UPLOAD' ? (
                            <Button
                                onClick={() => {
                                    router.post('/paraf-request', {
                                        _method: 'patch',
                                        sertifId: sertifikat.id,
                                    });
                                }}
                            >
                                Request paraf
                            </Button>
                        ) : sertifikat.paraf_bem === 'REQUEST' ? (
                            <h1>Sertifikat anda masih ditinjau</h1>
                        ) : (
                            <h1>Sertifkat anda sudah diapprove</h1>
                        )}
                    </div>
                );
